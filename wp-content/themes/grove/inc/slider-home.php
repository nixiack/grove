...
<?php } else {?>
	...
	if($slide_type == 'posttype') {
		$mmm = 'wooslider category="'.get_option('slide_pcat').'" slider_type="posts" layout="text-'.get_option('slide_text_side').'" limit="'.get_option("slide_num").'"';
		...
		$mmm = 'wooslider slide_page="'.get_option("slide_page").'" slider_type="slides" smoothheight="true" thumbnails="'.get_option("slide_pagination").'" order="DESC" order_by="'.get_option("slide_order_by").'" limit="'.get_option("slide_num").'"';
